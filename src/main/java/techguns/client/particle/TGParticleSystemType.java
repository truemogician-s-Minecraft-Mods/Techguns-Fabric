...
public class TGParticleSystemType extends TGFXType{
	RenderType renderType = RenderType.SOLID; //Type of Transparency
	Identifier texture = new Identifier("techguns:textures/fx/fireball.png"); //Particle Texture
...
//				}
	...
	public static IVolumeType VOL_CYLINDER2 = new IVolumeType() { //Tilted cylinder
		...
		public Vec3d getPosition(TGParticleSystem sys, DirResult dir, int i, int count) {
			...
			if (sys.type.volumeData.length >= 2) {
				...
				Vec3d direction = new Vec3d(0.0, 1.0, 0.0).rotateX(angle); //.rotateYaw((float)(Math.PI*0.5d));
				dir.setValues((float)direction.x, (float)direction.y, (float)direction.z);
		...
		public Vec3d getPosition(TGParticleSystem sys, DirResult dir, int i, int count) {
			...
			if (sys.type.volumeData.length >= 1) {
				...
				if (ent==null){
					...
					Vec3d motion = new Vec3d(ent.getVelocity().x, ent.getVelocity().y, ent.getVelocity().z).normalize();
	...
	@Override
	public List<TGParticleSystem> createParticleSystemsOnEntityItemAttached(Entity ent, Hand hand) {
		//TODO itemAttached particles
