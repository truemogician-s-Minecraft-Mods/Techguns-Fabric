...
public class GunAnimation {
	...
	abstract class Transformation {
		public abstract void apply(MatrixStack m, float f, boolean mirror);
	}
	...
	class Translate extends Transformation {
		...
		@Override
		public void apply(MatrixStack m, float f, boolean mirror) {
			m.translate(mirror  ? (x*-f) : (x*f), y*f, z*f);
		}		
	}
	...
	class Rotate extends Transformation {
		...
		@Override
		public void apply(MatrixStack mat, float f, boolean mirror) {
			TGMatrixOps.rotate(mat, f*angle, x, mirror ? -y : y, mirror ? -z : z);
		}		
	}	
	
	public void play(MatrixStack mat, float progress, boolean mirror, float... magnitudes) {
		float prev = 0.0f;
		float value = 0.0f;
